...
animalCases <- read.csv("Output/animal_cases.csv",row.names = NULL)
...
animalCases_sub <- dplyr::select(animalCases, Year, Date.bitten, Symptoms.started, "x"=UTM.Easting, "y"=UTM.Northing) %>% mutate("Source"="Cases")
humanExposures_sub <- dplyr::select(humanExposures, "Year"=Year.bitten, "x"=UTM.Easting, "y"=UTM.Northing) %>% mutate("Source"="Exposures")
humanDeaths_sub <- dplyr::select(humanDeaths, "Year"=Year.bitten, "x"=UTM.Easting, "y"=UTM.Northing) %>% mutate("Source"="Deaths")
