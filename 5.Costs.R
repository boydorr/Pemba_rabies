...
human <- read.csv("Output/human_bites.csv", row.names = NULL)
costs <- read.csv("data/costs.csv") # cost data
...
#' _How many people that were bitten by dogs (rabid and health) completed PEP?_
#' _and HOW MANY RABIES DEATHS PREVENTED?_
pRabies <- 0.17
bites_y$PEP3/bites_y$PEP1
bites_y$PEP3 # Annual people who completed PEP
bites_y$PEP1 # annual bites who started  PEP
...
#'depends on the PEP regimen (doses) and cost of vials, wound cleaning & health workers time etc
# parameters and their estimated costs, Direct (health provider) costs
...
#IM_dose = 4 # Number of pep doses required to prevent rabies
#ID_dose = 3 # Number of pep doses required to prevent rabies
...
# Identify what proportion of bites given IM vs ID vaccination
...
#'How much costs would the government spend on patients (PEP)in the absence of rabies?
#'Translate them into deaths averted
...
cost_per_death_avert <- govt_cost/deaths_averted_PEP # 158.3 USD  per deaths averted
cost_per_death_avert # With 95% CIs -  Mean, lower bound and upper bound is presented (because this is probability, predicting deaths in absence of PEP!)
...
# WHEN DOG RABIES IS ELIMINATED.... still get dog bite presentations from healthy dog bites...
# Govts always have to pay this residual PEP cost, PLUS annual MDV  - TOGETHER = One Health preventative costs....
rabies_free_PEP # annual cost of precautionary PEP after rabies eliminated 

# Estimate how much dog vaccination averts annual deaths and exposures risks
# then calculate the cost effectiveness of mass dog vaccination in averting exposure risks
...
#' _Description of cost table_
cost_variable <- c("Dog vaccine", "Syringe and needles", "Vaccination book registers", "Transport", "Advertisement",
                  "Fuel", "CAHW Allowance", "LFO allowance", "Overhead", "Post Exposure Prophylaxes (PEP)",
                  "Heath worker's time cost")
unit_cost <- c(1500, 123, 10000, 15000, 10000, 10000, 20000, 80000, 25000, 25200, 728)
cost_description <- c("Cost of dog vaccine", "Cost of syringe and needle", "Cost of dog vaccination registers",
                      "Vaccine transportation cost", "Cost of advertising dog vaccination campaign", "Fuel of vaccinators to reach vaccination station",
                      "Cost of vaccination assistant during vaccination campaign", "Cost of vaccinators during vaccination campaign",
                      "Medical consultation and first aid cost", "Cost of human vaccine", "Time cost of of the health workers to administer PEP")
cost_mode <- c("Cost per dose", "Per piece (PC)", "Per PC", "Public transport fare", "Advertisement per vaccination station",
               "Fuel per vaccination station", "Per day", "Per day", "Per patient", "Per vial", "Minutes per average salary")
cost_source <- c("Ministry of Livestock Developement", "Medical Store Department (MSD) price catalogue", "NA", "Land Transport Regulatory Authority (LATRA)", "NA", "District veterinary departmental rate", "Gates project budget allowance", "Gates project budget perdiem",
                 "NHIF health Insurance scheme", "MSD price catalogue", "Data calculation")

cost_table <- data.frame(cost_variable, unit_cost, cost_description, cost_mode, cost_source)

write.csv(cost_table, "Output/Tables/Table_S1_Cost_description.csv", row.names=FALSE)
